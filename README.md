...
- **`deploy.yml`** – runs on every push to `main`, builds the site, deploys it to GitHub Pages, and checks every external link on the built site with `lychee` (informational – it doesn't block the deploy). To republish without a merge, run this workflow manually from the **Actions** tab.
...
├── .github/workflows/
│   ├── deploy.yml               # Build + deploy to GitHub Pages (includes stored PR previews, external link check)
