...
export const ApiVersionAndBaseUrl = () => {
  const { siteConfig } = useDocusaurusContext();
  return (
    <>
      <strong>Version:</strong> {siteConfig.customFields.apiVersion} · <strong>Base URL:</strong> <code>{siteConfig.customFields.apiBaseUrl}</code>
    </>
  );
};

<ApiVersionAndBaseUrl />
